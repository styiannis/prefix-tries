import {
  ...
  deleteWord,
  size,
} from '../../src/core/compressed-trie';
import { entries } from '../../src/core/trie';
...
describe('core >> compressed-trie', () => {
  ...
  beforeAll(() => {
    expect(isValidObjectInstance(instance, 'compressed-trie')).toBe(true);
    expect(isValidObjectInstance(instance.root, 'compressed-trie-node')).toBe(
      true
    );
  ...
  it('Insert and search words [1]', () => {
    ...
    WORDS_1.forEach((word, i) => {
      ...
      expect(add(instance, word)).toBe(undefined);
      expect(includes(instance, word)).toBe(true);
      expect(size(instance)).toBe(WORDS_1.length);
    });
    ...
    WORDS_1.forEach((word, i) => {
      ...
      expect(deleteWord(instance, word)).toBe(true);
      expect(includes(instance, word)).toBe(false);
      expect(size(instance)).toBe(WORDS_1.length - i - 1);

      const iter = entries(instance);
      let j = 0;
      for (let word of iter) {
        expect(word).toBe(WORDS_1[1 + i + j]);
        i++;
      }
    });
  });
  ...
  it('Insert and search words [2]', () => {
    ...
    WORDS_2.forEach((word, i) => {
      ...
      expect(add(instance, word)).toBe(undefined);
      expect(includes(instance, word)).toBe(true);
      expect(size(instance)).toBe(WORDS_2.length);
    });
    ...
    // Try to deleteWord values ​​that are not included
    expect(deleteWord(instance, 'rubi')).toBe(false); // Valid prefix, invalid word
    expect(deleteWord(instance, 'invalid')).toBe(false); // Invalid prefix
    ...
    WORDS_2.forEach((word, i) => {
      ...
      expect(deleteWord(instance, word)).toBe(true);
      expect(includes(instance, word)).toBe(false);
      expect(size(instance)).toBe(WORDS_2.length - i - 1);

      const iter = entries(instance);
      let j = 0;
      for (let word of iter) {
        expect(word).toBe(WORDS_2[1 + i + j]);
        i++;
      }
    });
  });
